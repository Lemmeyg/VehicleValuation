...
export async function POST(request: NextRequest) {
  ...
  // on its own, but we guard here too to match the pattern above). Only stamp
  // the leads row when Zoho actually accepted the contact, so the backstop
  // cron (/api/cron/dispute-letter-recovery) can find and retry anything missed.
  try {
