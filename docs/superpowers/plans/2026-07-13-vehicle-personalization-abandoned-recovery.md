...
- This plan depends on the not-yet-built N2 cron (`docs/superpowers/plans/2026-07-10-abandoned-report-recovery-emails.md`, which creates `app/api/cron/abandoned-report-recovery/route.ts` and `lib/zoho-campaigns.ts`). **Status: N2 is already merged (PR #88) — this dependency is satisfied.**
- Do not implement this plan until the current single-code-entry version of the 3-email drip is built, live in Zoho, and running — deliberate fast-follow, not a launch blocker. **Status as of 2026-07-16: the drip is live, but the Zoho integration is not working correctly and needs separate fix/setup work. Tasks 6-7 are deferred until that's resolved — implement Tasks 1-5 and Task 8 only for now.**
